...
// Helper to get server status hover information
const getServerStatusHoverInfo = (server: MCPServer): string => {
    switch (server.status) {
        ...
        case 'error':
            return `Error: ${server.errorMessage || 'Connection failed'}`;
        default:
...
const ServerItem = ({ server, isSelected, onToggle, onRemove, onEdit, onToggleStatus, onRestart }: {
    server: MCPServer,
    isSelected: boolean,
    onToggle: () => void,
    ...
    // Determine if this is the DexPaprika server
    const isDexPaprika = server.id === 'dexpaprika-mcp';
    
    return (
        <div 
            className={`px-2 py-2 rounded-lg flex items-start gap-2 ${
                isSelected 
        ...
        >
            <div className="flex-shrink-0 pt-1">
                <Button
                    variant="ghost"
                    size="icon"
                    className={`h-5 w-5 rounded-full ${isSelected ? 'text-primary' : 'text-muted-foreground'}`}
                    onClick={onToggle}
                >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                    ...
                    {/* Special badge for DexPaprika */}
                    {isDexPaprika && (
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-amber-500/20 text-amber-500 border border-amber-500/30">
                            DEX Data
                        </span>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between mt-1">
                    ...
                    <StatusIndicator 
                        ...
                        onClick={server.status === 'error' ? () => onToggleStatus && onToggleStatus({} as React.MouseEvent) : undefined}
            ...
            <div className="flex-shrink-0 flex gap-0.5">
                ...
                <Button
                    ...
                    onClick={onRestart}
                    disabled={!(server.status === 'connected' || server.status === 'error')}
                                                        ...
                                                        <div className="flex items-center justify-between mb-2">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <div className="flex items-center">
                                                                    ...
                                                                    >
                                                                        <RefreshCcw className={`h-3.5 w-3.5 text-muted-foreground ${server.status === 'connecting' ? 'opacity-50' : ''}`} />
                                                                    </button>
