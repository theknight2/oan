...
import { MessageSquare, PlusCircle, Trash2, Sparkles, ChevronsUpDown, Copy, Pencil } from "lucide-react";
import {
    Sidebar,
    SidebarContent,
    SidebarFooter,
...
export function ChatSidebar() {
    ...
    return (
        <Sidebar className="shadow-sm bg-background" collapsible="icon">
            <SidebarHeader className="p-4 border-b border-border/40">
                <div className="flex items-center justify-start">
                    <div className={`flex items-center gap-2 ${isCollapsed ? "justify-center w-full" : ""}`}>
                        <div className={`relative flex items-center justify-center ${isCollapsed ? "size-5" : "size-7"}`}>
                            <Image 
                                src="/alpha-logo.svg" 
                                alt="OpenΑlpha Logo" 
                                width={28} 
                        ...
                        {!isCollapsed && (
                            <div className="font-semibold text-lg text-foreground/90">OpenΑlpha</div>
                        )}
                    </div>
                </div>
            </SidebarHeader>
...
{/* MCP Servers section removed as requested - these are now preconfigured without user access */}
            </SidebarContent>
